...
st.set_page_config(
  ...
  menu_items={
        ...
        'About': "# JONATHAN LONDOÑO Y SOFIA HIGUERA. Inteligencia Artificial  Deployment!"
    }
  )
...
st.header('Machine Learning para predecir ingresos', divider='rainbow')
...
with st.container( border=True):
  st.subheader("Modelo Machine Learning ")
  #Se desea usar emoji lo puedes buscar aqui.
  st.write("Realizado por Jonathan londoño y Sifua higuera")
  st.write("""
...
with st.container(border=True,height=300):
  ...
  st.write(""" Este es un ejemplo de despliegue de los modelos de Machine Learning entrenados en
           ...
           y scores obtenidos.
          https://colab.research.google.com/drive/1y2-CXJ2VqqvBltVjZJpJe8a22uuE5hQJ#scrollTo=YDBqWr8kNG3u""")
  ...
  with left_column:
    ...
    st.write(
      ...
      'edad', 'clase_trabajo', 'num_educacion', 'estado_civil', 'ocupacion', 'relacion', 'raza', 'sexo', 'horas_por_semana' , 'pais_natal'.
...
churn = {1 : 'Mayor o igual a 50k', 0 : 'Menor o igual a 50k' }
...
def seleccionar(modeloL):
  ...
  st.sidebar.subheader('Seleccione la edad')
  edad=st.sidebar.slider("Seleccion",0,100,56,5)
  
  #Filtrar por PROM
  st.sidebar.subheader('Selector del clase_trabajo')
  clase_trabajo=st.sidebar.slider("Seleccion",   1, 100,5.5,2)
  
  #Filtrar por COMINT
  st.sidebar.subheader('Selector de num_educacion')
  num_educacion=st.sidebar.slider("Seleccione",1,20,4,1)
  
  #Filtrar por COMPPRES
  st.sidebar.subheader('Selector de estado_civil') 
  estado_civil=st.sidebar.slider('Seleccione', 0,10,1,1)
  
  #Filtrar por RATE
  st.sidebar.subheader('Selector de ocupacion')
  ocupacion=st.sidebar.slider("Seleccione",0,20,12,1)

  #Filtrar por DIASSINQ
  st.sidebar.subheader('Selector de relacion')
  relacion=st.sidebar.slider("Seleccione", 0,10,4,1)
  
    #Filtrar por TASARET
  st.sidebar.subheader('Selector de raza')
  raza=st.sidebar.slider("Seleccione",0,8,4,1)
  
    #Filtrar por NUMQ
  st.sidebar.subheader('Selector de sexo')
  sexo=st.sidebar.slider("Seleccione",0,1,1,1)
  
    #Filtrar por NUMQ
  st.sidebar.subheader('Selector de horas_por_semana')
  horas_por_semana=st.sidebar.slider("Seleccione",0,100,18,2)
   #Filtrar por NUMQ
  st.sidebar.subheader('Selector de pais_natal')
  pais_natal=st.sidebar.slider("Seleccione",0,15,2,1)

  return modeloS,edad, clase_trabajo, num_educacion ,estado_civil, ocupacion, relacion,raza, sexo, horas_por_semana,pais_natal


# Se llama la función, y se guardan los valores seleccionados en cada variable

modelo,modeloS,edad, clase_trabajo, num_educacion ,estado_civil, ocupacion, relacion,raza, sexo, horas_por_semana,pais_natal=seleccionar(modeloA)
...
with st.container(border=True):
  ...
  lista=[[modeloS,edad, clase_trabajo, num_educacion ,estado_civil, ocupacion, relacion,raza, sexo, horas_por_semana,pais_natal]]
  X_predecir=pd.DataFrame(lista,columns=['edad', 'clase_trabajo', 'num_educacion', 'estado_civil', 'ocupacion', 'relacion','raza', 'sexo', 'horas_por_semana','pais_natal'])
